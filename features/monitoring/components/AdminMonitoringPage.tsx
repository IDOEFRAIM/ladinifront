...
import { MetricsGrid, AgentHealthGrid, ActionApprovalCard, ConversationTimeline, MonitoringFiltersPanel, AgentActivityFeed } from '@/features/monitoring/components';
import { useAdminMonitoringView, useAgentActions, useConversations, useMonitoringSSE } from '@/features/monitoring/hooks/useAgentMonitor';
...
type AdminView = ReturnType<typeof useAdminMonitoringView>['data'];
type ActionsData = ReturnType<typeof useAgentActions>['data'];
type ConvsData = ReturnType<typeof useConversations>['data'];
type SetPagination = React.Dispatch<React.SetStateAction<PaginationParams>>;
type SetFilters = React.Dispatch<React.SetStateAction<MonitoringFilters>>;
...
function OverviewTab({ adminView, viewLoading, handleActionApproved }: { adminView: AdminView; viewLoading: boolean; handleActionApproved: (a: AgentAction) => void }) {
  return (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2" style={{ gap: 24 }}>
        <div>
          <h2 style={{ fontSize: 16, fontWeight: 700, color: C.forest, fontFamily: F.heading, marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
            Actions en Attente
            {(adminView?.pendingActions?.length ?? 0) > 0 && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {viewLoading ? (
              ...
              (adminView?.pendingActions ?? []).map((action) => (
...
function ActionsTab({ actionFilters, setActionFilters, actionsLoading, actionsData, setActionPagination, handleActionApproved }: { actionFilters: MonitoringFilters; setActionFilters: SetFilters; actionsLoading: boolean; actionsData: ActionsData; setActionPagination: SetPagination; handleActionApproved: (a: AgentAction) => void }) {
  return (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {actionsLoading ? (
          ...
          <>
            ...
            {actionsData?.data.map((action) => <ActionApprovalCard key={action.id} action={action} isAdmin onApproved={handleActionApproved} />)}
            {actionsData?.hasMore && (
              <div style={{ textAlign: 'center', paddingTop: 16 }}>
                <button onClick={() => setActionPagination((p) => ({ ...p, cursor: actionsData.nextCursor || undefined }))}
...
function ConversationsTab({ convFilters, setConvFilters, convsLoading, convsData, setConvPagination }: { convFilters: MonitoringFilters; setConvFilters: SetFilters; convsLoading: boolean; convsData: ConvsData; setConvPagination: SetPagination }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {convsData?.hasMore && (
        <div style={{ textAlign: 'center', paddingTop: 16 }}>
          <button onClick={() => setConvPagination((p) => ({ ...p, cursor: convsData.nextCursor || undefined }))}
            style={{ padding: '10px 24px', borderRadius: 100, border: 'none', background: 'rgba(6,78,59,0.06)', color: C.forest, fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: F.body, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
            Charger plus <ArrowRight size={14} />
          </button>
        </div>
...
function HealthTab({ adminView, viewLoading }: { adminView: AdminView; viewLoading: boolean }) {
