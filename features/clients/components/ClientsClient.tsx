...
async function fetchClientsViaApi(): Promise<Client[]> {
...
export default function ClientsClient({ initialClients = [], serverCreateClient }: { initialClients?: Client[]; serverCreateClient?: (c: any) => Promise<any> }) {
  const [searchTerm, setSearchTerm] = useState('');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [clients, setClients] = useState<Client[]>(initialClients || []);
  const [loading, setLoading] = useState(!initialClients || initialClients.length === 0);

  React.useEffect(() => { if (initialClients && initialClients.length > 0) { setClients(initialClients); setLoading(false); } else { fetchClientsViaApi().then(data => { setClients(data); setLoading(false); }); } }, [initialClients]);
  ...
  const handleAddClient = async (newClient: Omit<Client, 'id' | 'totalOrders' | 'totalSpent' | 'lastOrderDate'>) => {
    try {
      if (typeof serverCreateClient === 'function') {
        const created = await serverCreateClient(newClient);
        if (created) setClients(prev => [created, ...prev]);
      } else {
        const res = await axios.post('/api/clients', newClient, { withCredentials: true });
        if (res.status >= 200 && res.status < 300) fetchClientsViaApi().then(data => setClients(data));
      }
    } catch (e) {
