...
export const getCategories = async (): Promise<Category[]> => {
    try {
        if (typeof window === 'undefined') {
            const mod = await import('@/features/products/actions/get-catalogue-products');
 ...
 */
export const getProducts = async (filters: ProductFilters & { search?: string } = {}): Promise<Product[]> => {
    try {
        const params = new URLSearchParams();
        if (filters.category && filters.category !== 'all') params.append('category', filters.category);
        if (filters.region && filters.region !== 'all') params.append('region', filters.region);
        // Support both `searchQuery` (older code) and `search` (client uses `search` key)
        const searchValue = filters.searchQuery ?? filters.search;
        ...
        if (typeof window === 'undefined') {
            ...
            // Les colonnes numeric arrivent en chaîne depuis la base ; le contrat Product les déclare number (comportement historique).
    ...
    try {
        ...
        if (typeof window === 'undefined') {
            ...
            const formattedProduct = {
                ...
                unit: product.unit ?? '',
                quantity: Number(product.quantityForSale ?? 0),
                stock: Number(product.quantityForSale ?? 0),
                description: product.description || '',
                images: Array.isArray(product.images) ? product.images : [],
                audioUrl: product.audioUrl || null,
                status: Number(product.quantityForSale ?? 0) > 0 ? 'active' : 'sold_out',
                location: {
