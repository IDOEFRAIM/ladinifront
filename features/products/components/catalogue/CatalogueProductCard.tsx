...
interface ProductCardProps { product: Product; viewMode?: 'grid' | 'list'; priority?: boolean; }

function ProductImageSection({ product, isOutOfStock, priority }: { product: Product; isOutOfStock: boolean; priority?: boolean }) {
    return (
        <div style={{ height:220, position:'relative', overflow:'hidden', background:`${C.forest}08` }}>
            <Image
                src={product.images && product.images.length > 0 ? normalizeAssetUrl(product.images[0], 'products') : "/images/no_image.webp"}
                alt={product?.name || 'Product picture'}
                fill
                priority={priority}
                sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 300px"
                style={{ objectFit:'cover', filter: isOutOfStock ? 'grayscale(1)' : 'none' }}
            />
            <div style={{
...
export default function ProductCard({ product, viewMode = 'grid', priority }: ProductCardProps) {
    const p: any = product;
    ...
    return (
        <motion.div whileHover={{ y: -6 }} transition={{ type:'spring', stiffness:300 }} style={{ height:'100%' }}>
            ...
            >
                ...
                >
                    <ProductImageSection product={product} isOutOfStock={isOutOfStock} priority={priority} />
