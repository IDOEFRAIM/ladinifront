...
import ProductCard from '@/features/products/components/catalogue/CatalogueProductCard';
import UnifiedFilter from '@/features/products/components/catalogue/CatalogueFilter';
...
export default function CatalogueClient({ initialProducts = [], initialCategories = [], initialRegions = [], serverLoad, serverFetchFilters }: any) {
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      ...
      if (serverLoad) {
        const res = await serverLoad(params);
        setProducts(res || []);
      } else {
        // Fallback to client-side service (existing behaviour)
        const mod = await import('@/features/products/services/catalogue.service');
        const data = await mod.getProducts(params);
        setProducts(data || []);
      }
    } catch (e) {
      setProducts([]);
  ...
  const CatalogToolbar = ({ search, setSearch, viewMode, setViewMode, totalProducts, showMobileFilter, setShowMobileFilter }: any) => (
    <div style={{ background: C.glass, backdropFilter:'blur(20px)', WebkitBackdropFilter:'blur(20px)', borderRadius:24, border:`1px solid ${C.border}`, padding:'16px 24px', marginBottom:24, display:'flex', alignItems:'center', gap:16, flexWrap:'wrap' }}>
      ...
      <div style={{ display:'flex', gap:8 }}>
        <span style={{ fontFamily:F.body, fontSize:'0.82rem', color:C.muted, fontWeight:600, alignSelf:'center', marginRight:8 }}>{products.length} produits</span>
  ...
  const EmptyState = () => (
    <div style={{ background: C.glass, backdropFilter:'blur(20px)', WebkitBackdropFilter:'blur(20px)', borderRadius:24, border:`1px solid ${C.border}`, padding:64, textAlign:'center' }}>
